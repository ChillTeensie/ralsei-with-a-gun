...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		{
			case 'gun':
				dialogue = CoolUtil.coolTextFile(Paths.txt('gun/dia'));
		...
		switch(stageCheck)
		{
			...
			{
				defaultCamZoom = 0.7;
				curStage = 'ralseiStage';
				var bg:FlxSprite = new FlxSprite(-209.05, -282.25).loadGraphic(Paths.image('Background','ralsei'));
				bg.antialiasing = true;
		...
		switch (SONG.player2)
		{
			...
			case 'spirit':
				...
				camPos.set(dad.getGraphicMidpoint().x + 300, dad.getGraphicMidpoint().y);
			case 'ralsei':
				dad.setPosition(399.55, 443.95);	  //lol
				camPos.set(dad.getGraphicMidpoint().x + 420, dad.getGraphicMidpoint().y + -400);
		}
		...
		switch (curStage)
		{
			...
			case 'schoolEvil':
				...
				gf.y += 300;
			case 'ralseiStage':
				boyfriend.setPosition(1088.65, 664.5);
		}
		...
		healthBar.createFilledBar(0xFF6FF1B5, 0xFF31B0D1);
		// healthBar
		add(healthBar);
	...
	private function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				if (gottaHitNote)
					swagNote = new Note(daStrumTime, daNoteData, oldNote, false, false, daType, true);
				else
					swagNote = new Note(daStrumTime, daNoteData, oldNote, false, false, daType);
		...
		for (i in 0...4)
		{
			...
			switch (noteTypeCheck)
			{
				case 'pixel':
					...
					case 'normal':
						if (player == 0)
							babyArrow.frames = Paths.getSparrowAtlas('ralsei_notes', 'ralsei');
						else 
